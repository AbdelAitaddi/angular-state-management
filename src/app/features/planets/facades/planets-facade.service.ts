...
export class PlanetsFacadeService {
  /*
  checkCount = signal(1).asReadonly();

  double = computed(() => this.checkCount() * 2);

  secondsObs = interval(5000);
  constructor() {
    const effectRef: EffectRef = effect(() => console.log(this.double()));
    effectRef.destroy();

    const seconds = toSignal(this.secondsObs, { initialValue: 0 });
    effect(() => {
